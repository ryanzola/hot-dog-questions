<template>
  <section class="container mx-auto">
    <div class="card">
      <div class="header">
        <div class="title font-bold text-xl mb-2 text-blue uppercase">
          {{ questions[$route.params.question].title }}
        </div>
        <div class="dots">🌭 🌭 🌭</div>
      </div>
      ...
      <form v-if="$route.params.question === 'privacy'">
        ...
        <div class="button-thing-group one">
          <div class="button-thing">
            <label for="like-radio">{{
              questions[$route.params.question].buttonText
            }}</label>
            <input
              type="radio"
              id="like-radio"
              @click="onSubmit({ privacy: true })"
            />
          </div>
        </div>
      </form>
      ...
      <form v-if="$route.params.question === 'hotdog'">
        ...
        <div class="button-thing-group two flex justify-between">
          <div class="button-thing">
            <label
              for="like-radio-1"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[0] }}</label
            >
            <input
              type="radio"
              id="like-radio-1"
              @click="onSubmit({ hotdog: true })"
            />
          </div>
          <div class="button-thing">
            <label
              for="like-radio-2"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[1] }}</label
            >
            <input
              type="radio"
              id="like-radio-2"
              @click="onSubmit({ hotdog: false })"
            />
          </div>
        </div>
      </form>
        ...
        <div class="button-thing-group six">
          <div class="button-thing">
            ...
            <input
              type="radio"
              id="like-radio-1"
              @click="onSubmit({ toppings: 'relish' })"
            />
          </div>
          <div class="button-thing">
            <label
          ...
          <div class="button-thing">
            <label
              for="like-radio-3"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[2] }}</label
            >
            <input
              type="radio"
              id="like-radio-3"
              @click="onSubmit({ toppings: 'mustard' })"
            />
          </div>
          <div class="button-thing">
            <label
              for="like-radio-4"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[3] }}</label
            >
            <input
              type="radio"
              id="like-radio-4"
              @click="onSubmit({ toppings: 'ketchup' })"
            />
          </div>
          <div class="button-thing">
            <label
              for="like-radio-5"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[4] }}</label
            >
            <input
              type="radio"
              id="like-radio-5"
              @click="onSubmit({ toppings: 'chilli' })"
            />
          </div>
          <div class="button-thing disabled">
            <label
              for="like-radio-6"
              class="bg-blue hover:bg-blue-dark text-white font-bold py-2 px-4 rounded capitalize"
              >{{ questions[$route.params.question].options[5] }}</label
            >
            <input
              type="radio"
              id="like-radio-6"
              @click="onSubmit({ toppings: 'cheese' })"
            />
          </div>
        </div>
      </form>
...
export default {
  ...
  methods: {
    ...
    onSubmit(data) {
      ...
      this.$router.push(
        `/${state.parent ? state.parent + "/" : ""}${state.child}`
      );
    }
  },
